    ...
    <div class="container">
        <form class="signup-form" id="signupForm" method="post">
            ...
            if (isset($_POST["submit"])) {
                ...
                $query = mysqli_query($conn, "SELECT * FROM iq_tester WHERE Email='$email' AND Password='$old_password'");
                $row = mysqli_fetch_array($query);

                if ($row) {
                    $query = mysqli_query($conn, "UPDATE iq_tester SET Password='$new_password' WHERE Email='$email'");
                    if ($query) {
                        echo "<p style='color:green; text-align:center;'>Password reset successfully</p>";
                    } else {
                        echo "<p style='color:red; text-align:center;'>Password reset failed</p>";
                    }
                } else {
